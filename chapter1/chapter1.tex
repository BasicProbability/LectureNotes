...
Here is a second example of what one can do with probability theory. Assume you want to invest on the 
...
In the context of this course,we will achieve this by imposing any total order on the sample space and then identifying the
outcomes with the positions they occupy in the corresponding ordered list. In this spirit we let 
...
die example we might be only interested in whether the outcome is even or odd. Transferring this
to the urn metaphor we would colour the balls with odd numbers green and the balls
...
The fact that our event spaces are closed under the set complement operation is very convenient. Say I
...
\begin{Exercise} 
In the above party example, what is the sample space? What is the smallest possible event space?
...
\subsection{Some basic combinatorics}
...
is drawn, it is removed from the urn. Let us make this concrete with the above example. For position one
...
\begin{Definition}
The binomial coefficient $ \binom{n}{k} $ is defined as \\ $ \binom{n}{k} := \dfrac{n!}{(n-k)!\times k!} $ for $ 0 < n, 0 \leq l \leq n $. 
