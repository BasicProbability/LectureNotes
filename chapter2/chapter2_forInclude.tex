...
\begin{Definition}[Measure]\label{axioms}
...
What we want concretely is that in total we only count $ \omega $'s contribution to the overall probability once.
...
binomial coefficient to the left hand side in eq. \ref{InExProofStep1} and at the same time subtract 1 from the
right hand side. This gives us \ref{InExProofStep2}, which is the equality that we are going to proof
...
our $ p $ from eq. \ref{binomTheorem} which is $ -1 $ in this case. We still need to account for 
...
clear to you, please go over it again. $ \square $\bigskip
...
To convince yourself that this is correct, you may want to consult fig. \ref{Venn2}. Alternatively, you
...
Before we get into the math of conditional probabilities, let us try to understand what this concept
