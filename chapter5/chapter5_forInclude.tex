...
$$ P(X = x | \Theta = \theta) = g(\theta, t(x))h(x, t(x)) \ . $$
\end{Theorem}
paragraph{Proof} Assume $ t(x) $ is sufficient. Then
\begin{align}
&P(X = x | \Theta = \theta) = \\ 
&P(X=x|t(X) = t(x)) P(t(X) = t(x)| \Theta = \theta) = h(x, t(x))g(\theta, t(x)) \ . \nonumber
\end{align}
Now assume that $ P(X = x | \Theta = \theta) = g(\theta, v(x))h(x, v(x)) $ for some statistic $ v(x) $. We need to show
...
&= \sum_{x:v(x)=v}P(X=x,v(x)=v| \Theta = \theta) = \sum_{x:v(x)=v} g(v, \theta) h(x,v) \nonumber
\end{align}
...
\end{align}
Clearly, the conditional does not depend on $ \theta $ and thus $ v(X) $ is sufficient. 
\par
