...
from a binomial distribution with parameters $ n=10 $ and $ \theta=0.7 $. \chris{maybe we should actually *give* these data samples to make it more concrete? I think the graphs are a bit hard to understand like that.} Figure~\ref{fig:sparse_likelihood}
...
we can conclude that $ \frac{d}{d\theta}L_{x}(0) \not = 0 $ and $ \frac{d}{d\theta}L_{x}(1) \not = 0 $ at these points \textbf{Check this! \chris{do you mean the students, or me, Chris?}}. Hence, the only point at which the
