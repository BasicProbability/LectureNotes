...
statements about these 500 surveys\chris{is this the correct technical term? I thought that a single survey usually contains many data points}. The scope of your study does not extend beyond those 500 data points and all statements that you make
...
to gather information that may turn out to be valuable for making your predictions. Again, if you have compiled 500 surveys\chris{see above} from 
...
mean $ \mu $ as a parameter. To indicate that we are just making a guess at $ \mu $ we put a stroke on top\chris{overline it?}.
...
This result is huge! Before we interpret it, let us be clear about how we computed it. \chris{``We take the expectation with respect to
$ P_{X} $. We can do this because all RVs are identically distributed, namely according to $ P_{X} $.'' this is formally incorrect, the first three expectations are according to $P_{X_1^n}$ which happens to be $\prod_{i=1}^n P_X$. I think we should skip it entirely. } Lines \eqref{sampleMeanStep1} and
...
\end{Definition}
\chris{I think this is not correct. It seems to me that the constant 0 sequence $a_i =0$ fulfils this definition.}
\chris{It should be something like: for any $K \in \mathbb{R}$, there is a $n_0$ such that $\forall n \geq n_0 : |a_n| \geq K$.}
...
it is possible that a sequence does not have a limit at all. \chris{This is NOT captured by this definition!} We will not deal with this case here, though. 
...
\chris{I think we should stick to the more standard version: ``if for every $\eps >0$ there exists a $\delta >0$ such that $0<|x-x_0| \leq \delta$ implies $|L - f(x)| \leq \eps$''. I'm not totally sure if it's equivalent.}
...
a sample mean with low probability\chris{what is that?} has good chances of being reasonably close to the true mean.
...
a \emph{good} parameter vector\chris{all of a sudden, it's vector?}. Notice that exactly this learning problem is referred to when people talk about machine learning. What the machine is trying learn from data are \emph{good} parameters. Where the statistician would talk about
...
A maximum likelihood estimate (MLE) for a parameter set $ \Theta $ \chris{confusing because $\Theta$ was RV before. Do you mean $\supp(\Theta)$?} on a data sample $ x $ 
...
Notice further that in Figure~\ref{fig:likelihood_plots}, there is only one MLE for each of these functions. This is a nice property and we will shortly see why the likelihood functions have a unique maximum.
...
A maximum a posteriori (MAP) estimate for a parameter set $ \Theta $ \chris{$\supp(\Theta)$?} on a data sample $ x $ 
is a value $ \theta^{*} $ such that
...
After we have shown how to correctly derive the MLE from the MAP estimate, let us warn you about a seemingly convincing but faulty argument to show the same thing.
...
Equation~\eqref{eq:wrongDerivation} is not even well-defined. \chris{Hmm, by the same argument, you could argue that $\log(P(\Theta=\theta))$ is not well-defined: for a continuous distribution of $\Theta$, it's $\log(0)$??}
