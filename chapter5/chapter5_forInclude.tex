...
the data better than others. For example, the constant function $ \mathbb{6}(\cdot) $ which returns the
value 6 on all inputs delivers an extremely poor summary of the data (in fact it is not sensitive to the
...
concerned with in this script, which are all in the exponential family.}. If a statistics
...
\begin{Definition}[Sufficient Statistics (discrete)]
Given some discrete data $ x=x_{1}^{n} $ that has a distribution with parameters $ \theta $, 
a statistic $ t(x) $ is sufficient if $ P(X|T(x) = t(x)) $ is independent of $ \theta $.
\end{Definition}
...
$ P(X|\sum_{i=1}^{n} x_{i}) $. It takes the following form
\begin{equation*}
P\left( X=y|\sum_{i=1}^{n} x_{i} \right) = 
\begin{cases}
...
A statistic that is non-sufficient for the binomial distribution the value of the first outcome, i.e.
...
shows up in the left-hand-side and thus $ x_{1} $ is not a sufficient statistic for the binomial
distribution.

\begin{Exercise}
Assume a data set $ x=x_{1}^{n} $ whose observations are i.i.d. according to a Poisson distribution
with parameter $ \lambda $. Recall that the p.m.f. of the Poisson for one data point is
$$ P(X=x|\Lambda = lamda) = e^{-\lambda}\frac{\lambda^{x}}{x^{!}} \ . $$
Find the sufficient statistic for the Poisson.
\end{Exercise}

\textbf{TODO: ADD GENERAL DEFINITION THAT ALSO APPLIES TO DISCRETE DISTRIBUTIONS}
