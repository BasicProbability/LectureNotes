...
Notice that covariance is symmetric. Let us for a moment look at it as a function of $ X $ only. Then $ (Y - \E[Y]) = c \in \mathbb{R} $
serves as a coefficient that determines how much the covariance changes as we change $ P_{X} $. Under this view, the covariance would be a linear function
...
Another interesting question is to what extend two variables influence each other, i.e.\ how much the outcome of one
...
between two RVs (recall that the dependence per se is binary -- either the variables are dependent or they are not). There are different measures of correlation.
...
The correlation coefficient $ \rho $ measure the dependence between variables through their (co)variances. If two RVs $ X $ and $ Y $ are highly dependent, one provides a 
lot of information about the other. This holds also true for functions of $ X $ and $ Y $. Clearly, variance is a function of an RV and thus the variances of 
highly dependent variables are informative of one another.\textbf{TODO} Notice that covariance and correlation are closely related, so much in fact that one sometimes sees the 
following definition of covariance.
\begin{equation}
