...
Notice that covariance is symmetric. Another interesting question is to what extend two variables influence each other, i.e.\ how much the outcome of one
...
The Pearson correlation coefficient between to RVs $ X $ and $ Y $ with joint distribution $P_{XY}$ is given as
...
marginal distributions.\chris{WHAT???} Notice that covariance and correlation are closely related, so much in fact that one sometimes sese the following definition of covariance.
\begin{equation}
