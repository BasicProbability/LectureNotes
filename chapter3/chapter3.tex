...
In the literature, often one of the two mentionings of $X$ is dropped from the
...
\begin{Definition}
The \textbf{variance} of a RV $ X $ is given by
$$ var(X) := \E[\, (X - \E[X])^{2} \, ] \, .$$
\end{Definition}
...
\end{equation}
\chris{Shouldn't we introduce the important term ``iid'' for \emph{independent
    and identically distributed} variables somewhere around here?}
