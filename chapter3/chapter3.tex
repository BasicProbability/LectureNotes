...
$ a = \binom{49}{6} \times 6 $ sequences of balls that we can draw (6 out of 49 numbered and 1 out of 6 coloured balls).
...
If our prediction $ v_{1}, \ldots, v_{7} $ are ordered in ascending fashion, then
\begin{equation*}
c_{i} = 
\begin{cases}
1 & \mbox{if } v_{i} = b_{i} \\
...
The unfairness of the lottery comes from the fact that the lottery gets to draw one out of $ \binom{49}{6} \times 6 $ possible sequences \textit{at random} whereas the
player has to decide on only one sequence ahead of time. 
There are $ \binom{6}{4}\binom{43}{2}\times 5 $ ways in which your predictions can get exactly 4 numbers right and the coloured ball wrong in any given lottery draw. 
This is because there are $ \binom{6}{4} $ to guess 4 out of 6 numbers correctly and $ \binom{43}{2} $ ways to choose any two out of the 43 numbers that were not 
drawn. Moreover, you can choose any of the $ 5 $ coloured balls that were not drawn. By the same reasoning, there are $ \binom{6}{3}\binom{44}{3}$ ways in which you get 3 
numbered balls and the coloured ball correct. In total there are 
\begin{equation}
\binom{6}{4}\binom{43}{2}\times 5 + \binom{6}{3}\binom{44}{3} = 314545
\end{equation}
ways to agree on four balls with a single lottery draw. 
...
random variables of interest. Since by definition the distribution has to fulfil conditions that would allow us to interpret it
as a probability measure on a probability space, we can use it without worrying
about the underlying sample space too much. \chris{hmmm, not sure about this statement}\philip{I tried to formulate this a bit
...
If we set $ X = (Z_{1}, \ldots, Z_{m}) $, we can recover $ P_{Z_{1}^{m}} $ from $ P_{Z_{1}^{m}Y} $ in the same way.\chris{so?}\philip{Sorry, I do not understand your comment here.} Likewise,
...
to $P_{Y|X}=P_Y$). Moreover, independence makes it much easier to calculate the expectation and variance of jointly distributed random variables. We have to be careful, though. Jointly distributed random variables do not take on a joint value. Rather, each of the joint RVs still takes on its own value.\chris{what do you mean?}
This is why in the joint case we have to compute the expectation of functions of the joint RVs.\philip{Is this better?} The standard choice in the case of expectation
...
compute that probability that you will be able to meet the customer's demand. Based on this you can evaluate whether or not it is reasonable to
accept the customer's order.\chris{I don't understand the example,
  Bernoulli would be about \emph{counting} the number of failures.}\philip{Is this one better?}
...
To approach this problem, let us start out from the simplest case, were we are only waiting for one success. This means that our sequence of trials ends in a success,
