...
the posteriors over the observations that were equal to the given outcome. For the pairs $ (\Sexpr{unique_data[1]},c_{1}) $
and $ (\Sexpr{unique_data[3]},c_{3}) $ this gives:
\begin{align}
\E[\id{\Sexpr{unique_data[1]},c_{1}} \mid  X = \Sexpr{unique_data[1]}, \Theta=\theta^{(0)}] &= \Sexpr{occurrences[1]} \times \Sexpr{posteriors[1,1]} = \Sexpr{binomPosteriors[1,1]} \\
\E[\id{\Sexpr{unique_data[3]},c_{3}} \mid X = \Sexpr{unique_data[3]}, \Theta=\theta^{(0)}] &= \Sexpr{occurrences[3]} \times \Sexpr{posteriors[3,3]} = \Sexpr{binomPosteriors[3,3]}
\end{align} 
