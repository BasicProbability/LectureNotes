...
incomplete\chris{It seems to me that \emph{incomplete} is not defined
  formally enough. What do we mean by it?}\footnote{In general, whenever you have complete data, analytic MLE computation is possible. The moment your data is incomplete, it becomes impossible.}. 
...
fractional \chris{what does ``fractional'' mean here?} and thus when treating them as observations we are handling a dataset with fractional observations. While this may be conceptually awkward
...
probabilities! Thus we simply \chris{The frequent use of ``simply'' is
  almost cynical here... we should avoid it.}need to multiply the posteriors per of each observed outcome with the number of times this outcome was
