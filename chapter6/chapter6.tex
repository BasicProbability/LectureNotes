...
incomplete\chris{It seems to me that \emph{incomplete} is not defined
  formally enough. What do we mean by it?}\philip{I extended the
  footnote with an explanation. Does that work?}\chris{That's helpful!}\footnote{Incomplete data is 
...
\philip{We thus see that the sums are marginalisation steps.}\chris{I think all this can be written in a concise way as follows:
\begin{align*}
P(X_1^n=x_{1}^{n} \mid \Theta= \theta) &= \sum_{j_1=1}^3 \sum_{j_2=1}^3 \cdots \sum_{j_n=1}^3 P(X_1^n=x_{1}^{n}, Y_1=c_{j_1}, \ldots, Y_n=c_{j_n} \mid \Theta= \theta) \\
...
take on $ |\mathcal{Y}| $ possible values $c_1, c_2, \ldots, c_{|\mathcal{Y}|}$\chris{or $y_1, y_2, \ldots$??}. Then the likelihood function is 
\begin{align}
L_{x}(\theta) = P(X=x \mid \Theta=\theta) = \underset{j=1}{\overset{|\mathcal{Y}|}{\sum}} P(X=x, Y=y_{j} \mid \Theta=\theta)
\end{align}
\chris{before, these values were called $c_j$...}
...
\begin{align}
&P(X_{1}=6,Y_{1} = c_{1}) = 0.25 \times P(X_{1}=6 \mid Y_{1}=c_{1},\Theta = 0.4) = 0.02786918 \\
&P(X_{1}=6,Y_{1} = c_{2}) = 0.5 \times P(X_{1}=6 \mid Y_{2}=c_{2},\Theta= 0.5)  = 0.1025391 \nonumber \\ 
&P(X_{1}=6,Y_{1} = c_{3}) =  0.25 \times P(X_{1}=6 \mid Y_{3}=c_{3},\Theta = 0.65)  = 0.05941712 \nonumber
\end{align}
...
P(Y_{1} = c_{1} \mid X_{1}=6,\Theta= \theta^{0}) = \frac{P(X_{1}=6,Y_{1} = c_{1} \mid \Theta= \theta^{0})}{P(X_1 = 6 \mid \Theta= \theta^{0})} = 0.146814 \\
