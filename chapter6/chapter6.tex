...
\begin{align} 
&P(X=x_{1}^{n},Y_{1}^{n}=y_{1}^{n}\mid \Theta=\theta) \label{eq:mixtureExample} \\
...
incomplete\chris{It seems to me that \emph{incomplete} is not defined
  formally enough. What do we mean by it?}\philip{I extended the footnote with an explanation. Does that work?}\footnote{Incomplete data is 
  ...
  an observed part (the number of heads in our example) and a latent part (the mixture components which in our example are the coins). In general, whenever you have complete data, analytic MLE computation is possible. The moment your data is incomplete, it becomes impossible.}. 
...
P(X=x_{1}^{n}| \Theta= \theta) &= \prod_{i=1}^{n} \sum_{j=1}^{3} P(X_{i}=x_{i},Y_{i}=c_{j}| \Theta= \theta)
\end{align}
...
\chris{this is going too fast! I don't understand it, you also
  ...
  on now.}
% \begin{align}\label{eq:marginal}
% P(X=x_{1}^{n}| \Theta= \theta) &= \sum_{j=1}^3 P(Y_i = c_j)
%                                  P(X=x_{1}^{n}|Y_{1}^{n}=y_{1}^{n},\Theta=\theta)
%   \\
% &=  \prod_{i=1}^{n} \sum_{j=1}^{3} P(X_{i}=x_{1},Y_{i}=c_{j}| \Theta= \theta)
% \end{align}

\philip{We thus see that the sums are marginalisation steps.} Now that we have related mixture models to the probability of the observed data, we can hope to estimate
...
fractional \chris{what does ``fractional'' mean here?}\philip{i.e. take on non-integer values} and thus when treating them as observations we are handling a dataset with fractional observations. While this may be conceptually awkward
...
\begin{align}
\E(Y = c_{1}|X=x,\Theta= \theta^{0}) = 2 \times 0.5674795 + 3 \times 0.4568744 \label{eq:c1Expectation} \\ 
...
$ \theta_{j}^{(1)} = \frac{\E[Y = c_{j}|X=x,\Theta= \theta^{0}]}{20} $\footnote{When implementing the algorithm, numerical imprecisions occur due the limited
memory of our computers. To ensure that the distribution obtained from the M-step really sums to 1, you should use the sum 
$ \sum_{j=1}^{3} \frac{\E[Y = c_{j}|X=x,\Theta= \theta^{0}]}{20} $. This sum should be equal to 20 up to at least the third decimal but may not be exactly equal to
...
\begin{equation}
\theta^{1}_{c_{j}} = \frac{\E[X=x|Y=c_{j},\Theta=\theta^{0}]}{\sum_{x \in \mathcal{X}}\E[X=x|Y_{i}=c_{j},\Theta=\theta^{0}]}\ .
\end{equation}
...
directly observe the number of success per draw from $ c_{1} $ and instead compute the number of expected successes. The number of 
...
\begin{kframe}
\begin{alltt}
\hlstd{newBinoms} \hlkwb{=} \hlkwd{matrix}\hlstd{(}\hlnum{0}\hlstd{,}\hlnum{1}\hlstd{, cols)}

\hlkwa{for} \hlstd{(j} \hlkwa{in} \hlnum{1}\hlopt{:}\hlstd{cols) \{}
        \hlstd{newBinoms[}\hlnum{1}\hlstd{,j]} \hlkwb{=} \hlstd{binomPosteriorExp[}\hlnum{1}\hlstd{,j]}\hlopt{/}\hlstd{(}\hlnum{10} \hlopt{*} \hlkwd{sum}\hlstd{(binomPosteriors[,j]))}
\hlstd{\}}

\hlstd{output} \hlkwb{=} \hlstd{newBinoms}
\hlkwd{colnames}\hlstd{(output)} \hlkwb{=} \hlkwd{c}\hlstd{(}\hlstr{"c_1"}\hlstd{,} \hlstr{"c_2"}\hlstd{,} \hlstr{"c_3"}\hlstd{)}
\hlkwd{kable}\hlstd{(output,} \hlkwc{align}\hlstd{=}\hlstr{"c"}\hlstd{)}
\end{alltt}
\end{kframe}
\begin{tabular}{c|c|c}
\hline
